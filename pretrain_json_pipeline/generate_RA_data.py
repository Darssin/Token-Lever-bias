...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate RA train/valid/test parquet files from user_sequence.txt and item_meta.with_sid.json."
    )
    ...
    parser.add_argument("--train_name", type=str, default="training_RA_train.parquet")
    parser.add_argument("--valid_name", type=str, default="training_RA_valid.parquet")
    parser.add_argument("--test_name", type=str, default="training_RA_test.parquet")
...
def extract_item_sequence(item_ids: list[str], user_id: str, item_meta: dict) -> list[dict]:
...
def build_dataset_entry(user_id: str, item_sequence: list[dict], tail_remove_count: int) -> dict | None:
    ...
    for item in description_items:
        item_descriptions.append(
            f'{item["sid"]}, its title is "{item["title"]}", its categories are "{item["categories"]}"'
        )
...
def generate_ra_data(
    ...
    for idx, line in enumerate(lines):
        ...
        user_id = elements[0]
        item_ids = elements[1:]
        item_sequence = extract_item_sequence(item_ids, user_id, item_meta)
        if not item_sequence:
            continue

        train_entry = build_dataset_entry(user_id, item_sequence, tail_remove_count=2)
        if train_entry:
            train_rows.append(train_entry)

        valid_entry = build_dataset_entry(user_id, item_sequence, tail_remove_count=1)
        if valid_entry:
            valid_rows.append(valid_entry)

        test_entry = build_dataset_entry(user_id, item_sequence, tail_remove_count=0)
        if test_entry:
