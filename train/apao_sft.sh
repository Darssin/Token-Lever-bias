...
OUTPUT_DIR="/mnt/cfs/chubaofs_ads_train_image/ouchuang/bias/0.6B_apao_test/"
USE_LORA=false

# mean + avg: 先算出每个位置的“前缀平均 loss”，再把这些前缀 loss 全部平均
# worst + avg: 先算出所有“前缀平均 loss”，然后只取其中最大的那个
# adaptive + avg: 使用自适应前缀权重更新，动态提高高损失前缀的权重
PREFIX_LOSS_WEIGHT=1.0
PREFIX_LOSS_TYPE="avg"
PREFIX_AGGREGATION="adaptive"
PREFIX_WEIGHT_ETA=1e-4
...
DEEPSPEED_CMD=(
    ...
    --prefix_weight_eta "${PREFIX_WEIGHT_ETA}"
)
...
nohup env WANDB_MODE=offline "${DEEPSPEED_CMD[@]}" 2>&1 | tee -a beauty_sid_rec_apao.log &
