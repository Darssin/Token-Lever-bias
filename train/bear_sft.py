...
@dataclass
class BEARArguments:
    bear_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for the BEAR regularization term."},
    )
    bear_temperature: float = field(
        default=1.0,
        metadata={"help": "Temperature xi used in the BEAR sigmoid surrogate."},
    )
    bear_beam_size: int = field(
        default=10,
        metadata={"help": "Beam size B used to construct the top-B token threshold."},
    )
...
"""
    ...
    if local_rank == 0:
        ...
        for i, text in enumerate(texts[:3]):
            ...
            print("      (Note: CE loss starts from assistant response; BEAR only covers the target item span)")
            print()
...
class CustomDataCollator:
    def __init__(self, tokenizer, mlm=False):
        ...
        self.im_end_tokens = self.tokenizer.encode(
            self.im_end,
            add_special_tokens=False,
        )
    ...
    def _find_target_span(self, ids: List[int]) -> Tuple[int, int]:
        response_start = self._find_subsequence(ids, self.response_prefix_tokens)
        if response_start != -1:
            target_start = response_start + len(self.response_prefix_tokens)
        else:
            assistant_start = self._find_subsequence(ids, self.assistant_prefix_tokens)
            if assistant_start == -1:
                return -1, -1
            target_start = assistant_start + len(self.assistant_prefix_tokens)

        target_end = self._find_subsequence(ids, self.im_end_tokens, start=target_start)
        if target_end == -1:
            target_end = len(ids)
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for ids, mask in zip(input_ids, attention_mask):
            ...
            bear_mask = [0] * len(padded_ids)

            target_start, target_end = self._find_target_span(ids)

            if target_start == -1:
                label = [-100] * len(label)
            else:
                for k in range(target_start):
                    label[k] = -100
                for k in range(target_start, min(target_end, len(bear_mask))):
                    bear_mask[k] = 1
            ...
            labels.append(label)
            bear_masks.append(bear_mask)

        return {
            "input_ids": torch.tensor(padded_input_ids, dtype=torch.long),
            "attention_mask": torch.tensor(padded_attention_mask, dtype=torch.long),
            "labels": torch.tensor(labels, dtype=torch.long),
            "bear_mask": torch.tensor(bear_masks, dtype=torch.bool),
        }
...
class BEARTrainer(Trainer):
    def __init__(
        ...
        *args,
        ...
        bear_temperature: float = 1.0,
        bear_beam_size: int = 10,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        if bear_temperature <= 0:
            raise ValueError("bear_temperature must be positive")
        if bear_beam_size <= 0:
            raise ValueError("bear_beam_size must be positive")

        self.bear_weight = bear_weight
        self.bear_temperature = bear_temperature
        self.bear_beam_size = bear_beam_size

    def _compute_ce_loss(self, shift_logits: torch.Tensor, shift_labels: torch.Tensor) -> torch.Tensor:
        ...
        return token_losses.masked_select(valid_mask).mean()

    def _compute_bear_loss(
        self,
        shift_logits: torch.Tensor,
        shift_labels: torch.Tensor,
        shift_bear_mask: torch.Tensor,
    ) -> torch.Tensor:
        valid_mask = shift_bear_mask & shift_labels.ne(-100)
        if not valid_mask.any():
            return shift_logits.new_zeros(())
        ...
        topk = min(self.bear_beam_size, log_probs.size(-1))
        topk_values = torch.topk(log_probs, k=topk, dim=-1).values
        threshold_log_probs = topk_values[..., -1]

        margins = threshold_log_probs - gold_log_probs
        bear_terms = F.logsigmoid(margins / self.bear_temperature)

        return bear_terms.masked_select(valid_mask).mean()

    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        labels = inputs["labels"]
        bear_mask = inputs["bear_mask"]
        ...
        shift_bear_mask = bear_mask[..., 1:].contiguous()

        ce_loss = self._compute_ce_loss(shift_logits, shift_labels)
        bear_loss = self._compute_bear_loss(shift_logits, shift_labels, shift_bear_mask)
        loss = ce_loss + self.bear_weight * bear_loss

        if return_outputs:
            outputs.loss = loss
            outputs.ce_loss = ce_loss.detach()
            outputs.bear_loss = bear_loss.detach()
            return loss, outputs
        return loss


if __name__ == "__main__":
    parser = HfArgumentParser((ModelArguments, DataArguments, BEARArguments, TrainingArguments))
    model_args, data_args, bear_args, training_args = parser.parse_args_into_dataclasses()
    training_args.label_names = ["labels", "bear_mask"]
    ...
    if training_args.local_rank == 0:
        ...
        print(f"BEAR weight = {bear_args.bear_weight}")
        print(f"BEAR temperature = {bear_args.bear_temperature}")
        print(f"BEAR beam size = {bear_args.bear_beam_size}")
    ...
    trainer = BEARTrainer(
        ...
        bear_weight=bear_args.bear_weight,
        bear_temperature=bear_args.bear_temperature,
        bear_beam_size=bear_args.bear_beam_size,
    )
